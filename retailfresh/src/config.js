var _domain = 'https://localhost:3000/oauth2/redirect.html';
var _metadata = `
eyJqa3UiOiJodHRwczovL2lkZW50aXR5Lm9hZGEtZGV2LmNvbS9jZXJ0cyIsImtpZCI6ImtqY1NjamMzMmR3SlhYTEpEczNyMTI0c2ExIiwidHlwIjoiSldUIiwiYWxnIjoiUlMyNTYifQ.-Il0sInNvZnR3YXJlX2lkIjoiYTQwMjY5NDAtMzQ2MS00ODM5LTkxMjMtNDQyYzk1OTU1MGRmIiwicmVnaXN0cmF0aW9uX3Byb3ZpZGVyIjoiaHR0cHM6Ly9pZGVudGl0eS5vYWRhLWRldi5jb20iLCJpYXQiOjE1MDc4MjI4MDl9.QeAjEXKf732OvLm7evXFsR4N2GAgWHnDM84AVxSagOK64gUId_oMkUHRSwoy8QfJ81d3sgt_yUOlTGCM9G8kwsD8e5rwFmmeULFrfpXgxekMn8bRYii8JxVNF4pfR5jzSdJO9D2DECAbIinGjDFGsjS--knqmjpZ6OPeBPdA0dY
`.trim()
var _devtoolsPort = 8585;
var _fpadDomains = [
  {
    displayText: 'RetailFresh',
    url: 'https://api.retailfresh.fpad.io'
  }
];

if (process.env.NODE_ENV === 'production') {
  //Production values
  _domain = 'http://retailfresh.fpad.io/oauth2/redirect.html';

	_metadata = `
eyJqa3UiOiJodHRwczovL2lkZW50aXR5Lm9hZGEtZGV2LmNvbS9jZXJ0cyIsImtpZCI6ImtqY1NjamMzMmR3SlhYTEpEczNyMTI0c2ExIiwidHlwIjoiSldUIiwiYWxnIjoiUlMyNTYifQ..wQdjeLB84ENaAyqpYh_mn__f1JGrscO4IGtbLfmIDo4IkYTBE4xvKMRDNhP7zmwRlxibxaQdVSSD9bJLt5W_K1RBvzDB7MysYN2n7o-B7sTLFR7fCG3u6aKoOVaFSWRHrOOReRS9ZskRn1pmqtw-_rdeeDgCuUuADhLTwNc4qyI
  `.trim()

  _devtoolsPort = 8585;

  _fpadDomains = [
    {
      displayText: 'vip3',
      url: 'https://vip3.ecn.purdue.edu'
    },
    {
      displayText: 'RetailFresh',
      url: 'https://api.retailfresh.fpad.io'
    }
  ];
}

if (process.env.NODE_ENV === 'prod-dev') {
  //DEV values on the production server
  _domain = 'http://retailfresh.fpad.io:8080/oauth2/redirect.html';

	_metadata = `
	eyJqa3UiOiJodHRwczovL2lkZW50aXR5Lm9hZGEtZGV2LmNvbS9jZXJ0cyIsImtpZCI6ImtqY1NjamMzMmR3SlhYTEpEczNyMTI0c2ExIiwidHlwIjoiSldUIiwiYWxnIjoiUlMyNTYifQ..doJZdcR5lN4YyKGZTnC5DesnIQ7txxjHT50-NHKJpCnPwc_FinZ0OXXaZ8HRn6bTvMCn2tFKbyC_gm5KZdiR4_smywAV2alxBmyfMgtu59xFbR_UJacoHVOx6wj8bsjmHkhpLUdB2peeRGDTsQvVjktLIo729wsbcaTgT22NVv0
  `.trim()

  _devtoolsPort = 8585;

  _fpadDomains = [
    {
      displayText: 'vip3',
      url: 'https://vip3.ecn.purdue.edu'
    },
    {
      displayText: 'RetailFresh',
      url: 'https://api.abcaudits.fpad.io'
    }
    {
      displayText: 'RetailFresh',
      url: 'https://api.retailfresh.fpad.io'
    }
  ];
}



export default {
  fpadDomain: fpadDomain,
  domain: domain,
  devtoolsPort: devtoolsPort
};
